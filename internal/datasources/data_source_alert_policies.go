...
type alertPoliciesDataSourceModel struct {
	...
	AlertPolicies []resources.AlertPolicyResourceModel `tfsdk:"alert_policies"`
}
...
func (d *alertPoliciesDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	state := alertPoliciesDataSourceModel{
		...
		AlertPolicies: make([]resources.AlertPolicyResourceModel, len(policies)),
	}
	for i := range policies {
		state.AlertPolicies[i] = resources.AlertPolicyResourceModel{
			Deployment: types.StringValue(deployment),
			PolicyType: types.StringValue(policies[i].PolicyType),
		}
		resp.Diagnostics.Append(resources.PolicyToModel(&policies[i], &state.AlertPolicies[i])...)
		if resp.Diagnostics.HasError() {
			return
		}
	}
