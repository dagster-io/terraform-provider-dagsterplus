...
func testAccCheckUserTokenDestroyed(name string) resource.TestCheckFunc {
	return func(_ *terraform.State) error {
		...
		// Listing tokens requires a user_id, which is not available at destroy-check time.
		// We cannot verify deletion; the import step already confirms the resource
		// round-trips correctly via ID alone.
		_ = c
		return nil
	}
}
