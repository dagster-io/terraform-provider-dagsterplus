...
func testAccCheckCustomMetricDestroyed(metadataKey string) resource.TestCheckFunc {
	return func(_ *terraform.State) error {
		...
		if err != nil {
			return fmt.Errorf("listing custom metrics: %w", err)
		}
...
// TestAccCustomMetricResource_disappears verifies Terraform detects drift when the metric is
// deleted outside of Terraform.
func TestAccCustomMetricResource_disappears(t *testing.T) {
	rName := "acc_tf_" + acctest.RandStringFromCharSet(10, acctest.CharSetAlpha)
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttrSet("dagsterplus_custom_metric.test", "id"),
					testAccCustomMetricDisappears("dagsterplus_custom_metric.test"),
				),
...
func testAccCustomMetricDisappears(resourceName string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		return c.DeleteCustomMetric(context.Background(), rs.Primary.ID)
	}
}
