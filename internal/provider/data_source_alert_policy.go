...
// alertPolicyDataSourceModel mirrors alertPolicyResourceModel but uses a pointer for
...
func (d *alertPolicyDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	// Convert to resource model to reuse policyToModel, then copy into data source model.
	rm := alertPolicyResourceModel{
		Deployment: config.Deployment,
		PolicyType: config.PolicyType,
	}
	resp.Diagnostics.Append(policyToModel(policy, &rm)...)
	if resp.Diagnostics.HasError() {
		return
	}
